...
使用者明講：「我需要 insight 跟 what should i do next accordingly」。他曾示範過他要的敘事樣板：媽媽切水梨邊把一整週的盤勢數字串成一條決策鏈——**每個數字後面永遠掛兩件事：代表什麼 + 下一步**。

所以以下欄位的每句敘述**必須跟著「數字 → 代表什麼 → 你/市場該怎麼做」三段式**：
- `opportunities[].why`、`opportunities[].headline`、`opportunities[].head_to_head.verdict / pick_rationale / skip_rationale`
- `morning_brief.one_liner`、`morning_brief.highlights[].detail`
- `market_pulse.summary`、`macro_context.narrative`
- `topics[].narrative`
- `holdings_analysis[].commentary`
- `budget_allocation.allocations[].rationale`、`budget_allocation.plan_summary`
- `action_checklist` 每一項、`learning_point.description`

**三段式是什麼**：
```
[具體數字/事實]  →  [這個數字代表什麼]  →  [所以使用者 / 市場 / 外資該/會怎麼做]
```

**好範例 ✅**：
- ❌ 壞：「PCB 族群持續受惠 AI 伺服器需求，建議關注。」（三者皆無）
- ✅ 好：「外資這週在台玻砸 187 億（數字）→ 玻纖布供不應求，CCL 上游卡關（代表什麼）→ 想跟外資同向，買 2383 台光電或 5469 瀚宇博，不是追 PCB 本體（下一步）。」

- ❌ 壞：「外資今日賣超 20 億，台股走弱。」（有數字但沒「代表什麼」、沒「下一步」）
- ✅ 好：「外資今日賣超 20 億，但台指期淨空單增加 1,468 口到 4.1 萬口（數字）→ 外資一邊買現貨一邊放空期貨=避險加重，代表他們自己也怕跌（代表什麼）→ 下週若大漲不要追，等空單降下來才是真的看多（下一步）。」

- ❌ 壞：「2330 台積電基本面穩健，值得長期持有。」（空話、沒數字、沒可執行動作）
- ✅ 好：「2330 PE 22 (🟢 合理)、EPS TTM 52.3、預估 EPS 58、外資連買 3 日 2.3 萬張（數字）→ AI Capex 旺季+CoWoS 供不應求，分析師上修沒停（代表什麼）→ 若你持有成本 800 現在 +44%，別急加碼；若還沒建倉，等 1,080-1,100 區間再分批（下一步）。」

- ❌ 壞：「融資餘額上升，行情過熱。」
- ✅ 好：「融資餘額 4,271 億，較前日 +47 億（數字）→ 散戶在借錢進場，是行情過熱訊號（代表什麼）→ 現在不追強勢股，等融資回落再進場雪球試水（下一步）。」

**判斷鏈必須成立**：
- 如果你提不出「代表什麼」，那個數字就不該出現（刪掉）
- 如果你提不出「下一步」，那這句就沒價值給使用者（刪掉或合併）
- 最忌「描述→描述→描述」三連，沒有結論

**連接詞建議**：「→」、「代表」、「意味著」、「所以」、「因此」、「換句話說」、「對你來說就是」、「翻成白話是」。

**不要用**：「值得關注」「需要留意」「可能有機會」這類沒動作的結尾——改成：「→ 要做 X」「→ 不追；等 Y 訊號」「→ 先不動，下週三法說再判斷」。
...
- `headline`：像券商報告標題，**帶具體數字 + 結論暗示**。例：「外資這週在台玻砸 187 億，玻纖布卡關傳上游」「聯亞、上詮連 3 日量增，1.6T 升級才剛啟動」
- `why`：**三段式必遵守**——數字 → 代表什麼 → 你該怎麼做。100-180 字，一段話講清楚。
  好範例：「Bloomberg 報導北美 hyperscaler Q1 capex 上修 18%、重電族群外資本週買超 52 億（數字）→ 台灣重電供應鏈才進入需求第二輪反應，PE 仍在 15-20 倍合理區（代表什麼）→ 想抓這題材，從 1519 華城、1503 士電、1590 亞德客這類中型供應商切入，不是追 2308 台達電（上游大型股漲幅已用盡）（下一步）。」
  壞範例：「AI 發現：重電族群正在反應 Capex 循環，估值仍有空間。」（沒數字、沒「代表什麼」、沒「下一步」）
...
- one_liner：**三段式必遵守**——承接 headline 的 40-80 字總結。格式：今日市場數字（大盤/外資/法人）→ 代表什麼 → 對使用者 net 是好/壞 + 今日最該做的一件事。
  ✅ 好：「大盤昨跌 327 點到 36,804、外資期貨空單 4.1 萬口（數字）→ 外資在加空避險，這是散戶追高要小心的位階（代表什麼）→ 你組合守得住（2330 +2.1%），今天最該做的是不要追任何 52 週創高的股票，等空單降下來再出手（下一步）。」
  ❌ 壞：「大盤收跌但組合尚穩，建議持續觀察。」（沒數字、沒下一步）
- highlights：**恰好 3 張** 顏色卡片，必須涵蓋 (a) win 今日進帳 (b) risk 要注意 (c) opp 機會。每張 `detail` 欄 **必須三段式**：數字 → 代表什麼 → 下一步，各段用「→」或「，」連接。
  ✅ 好 detail 範例：
    win:  "2330 +2.1%、外資買 1.2 萬張（數字）→ Capex 題材 AI 伺服器延續，還有 3 家券商上修 TP（代表什麼）→ 保留、不急加碼，接近 2,500 停利目標分批出（下一步）"
    risk: "融資餘額 4,271 億（+47 億）、外資空單 4.1 萬口（數字）→ 散戶進場+外資避險=過熱訊號（代表什麼）→ 今天別追任何強勢創高股，持有 3017/3324 請拉停損到 -8%（下一步）"
    opp:  "5469 瀚宇博近 5 日外資連買 3,200 張、PE 15.2 🟢（數字）→ CCL 上游漲價受惠、尚未被炒作（代表什麼）→ 下週一盤前觀察是否拉回，拉回 40 元以下就是小倉試水機會（下一步）"
  ❌ 壞 detail 範例：
    "持倉表現良好，值得關注。"（空話）
    "外資賣超 20 億，後市需觀察。"（沒下一步）
...
- **entry_price**（2026-04-19 新增）：一個具體的建議買入價位數字（例：`92.5`）。
  * 「新倉試水 / 加碼」→ 給盤中可接受的進場價（通常 ≈ 現價，也可以是「現價或拉回 X% 內」）
  * 「觀望等進場」→ 給你想等的拉回目標價（例：現價 100 → 進場價設 95）
  * **不可省略**。前端會把這個數字顯示在籃子卡片上，使用者照這個價位掛單。
- entry_zone（選填）：進場價區間文字（例：`88-92`），給使用者一個區間而不是單點
- entry_condition：進場條件敘述（**補充**用 — 解釋「為什麼是這個價位」）
  例：「盤前開盤價直接進」「拉回至 2500 以下再進」「等法說後觀察 3 天」
- stop_loss_price：建議停損價（通常 -8% ~ -12%，**距離現價至少 8%**）
...
def build_chips_context() -> str:
    """Inject chips.json into the prompt so Gemini has real 三大法人 streaks
    per stock + market-wide 外資期貨、融資餘額 to cite. Without this block
    Gemini has to hallucinate numbers when asked to ground claims in 籌碼.

    Structure:
      ## 籌碼資料（TWSE + TPEx + TAIFEX）
      ### 市場面（今日整體）
      - 外資期貨淨 OI: -41,151 口（空單，較昨日 -1,468）
      - 融資餘額: 4,271.2 億（較昨日 +47.1 億 — 散戶加碼）
      ### 個股 · 近 5 日買賣超（股）
      - 2330 台積電: 外資 +1.2M / 投信 +50k / 自營 -10k · 外資連買 3 日
      - 3081 聯亞:   外資 -200k / 投信 +80k            · 投信連買 5 日
    """
    chips_path = ROOT / "chips.json"
    if not chips_path.exists():
        return ""
    try:
        chips = json.loads(chips_path.read_text(encoding="utf-8"))
    except Exception:
        return ""

    out: list[str] = [
        "## 籌碼資料（三大法人 + 外資期貨未平倉 + 融資餘額）",
        "",
        "⚠️ **極重要規則 · Gemini 必讀**：",
        "- 任何 opportunities.why、head_to_head、morning_brief.highlights、action_checklist 的敘述"
        "**必須引用下方實際的法人買賣超數字、外資期貨空單口數、融資餘額**，",
        "  不要自己編造「外資連買 3 日」這種沒根據的幻覺。",
        "- 沒有籌碼資料的個股就寫「近 5 日法人動作待補」，不要瞎編。",
        "- 市場面數字（外資期貨、融資）是整個 market_pulse / macro_context / morning_brief 必要素材 —"
        "它們是「台股當下是過熱還冷卻」的決定性訊號。",
        "",
    ]

    # --- 市場面 ---
    mc = chips.get("market_chips") or {}
    fx = mc.get("foreign_futures") or {}
    mg = mc.get("margin_total") or {}
    if fx or mg:
        out.append("### 市場面（今日整體 · 這三個數字直接決定 macro_context 方向）")
        if fx:
            latest = fx.get("latest") or {}
            prev = fx.get("prev") or {}
            ch = fx.get("change_1d")
            net = latest.get("net_oi")
            if net is not None:
                direction = "空單淨額" if net < 0 else "多單淨額"
                bit = f"**外資台指期{direction}: {abs(net):,} 口**"
                if ch is not None and prev.get("net_oi") is not None:
                    trend = "空單加重（外資更保守）" if ch < 0 else "空單減少（外資在解避險）"
                    bit += f"（{latest.get('date')}；較前日 {ch:+,} 口 — {trend}）"
                out.append(f"- {bit}")
                # Add interpretive hint
                if net < -35000:
                    out.append("  → 這是外資「大幅避險中」的水位，代表他們一邊買現貨一邊放空期貨，不是真的看多。")
                elif net < -15000:
                    out.append("  → 這是外資「一般避險」水位，偏中性。")
                elif net > 15000:
                    out.append("  → 這是外資「偏多」水位，期貨淨多單罕見，代表他們真的看漲。")
        if mg:
            latest = mg.get("latest") or {}
            prev = mg.get("prev") or {}
            ch = mg.get("change_1d_yi")
            bal = latest.get("balance_yi")
            short_lots = latest.get("short_lots")
            if bal is not None:
                bit = f"**融資餘額: {bal:,.1f} 億**"
                if ch is not None:
                    trend = "散戶加碼融資（行情過熱訊號）" if ch > 0 else "融資下降（散戶退場）"
                    bit += f"（{latest.get('date')}；較前日 {ch:+.1f} 億 — {trend}）"
                out.append(f"- {bit}")
                if short_lots is not None:
                    out.append(f"- 融券餘額: {short_lots:,} 張")
        out.append("")

    # --- 個股層 ---
    by_sym = chips.get("by_symbol") or {}
    if by_sym:
        out.append("### 個股 · 近 5 日與近 20 日法人買賣超（單位：股；連買/連賣天數越長越具持續性）")
        # Load portfolio + watchlist for priority ordering
        hold_syms: list[str] = []
        watch_syms: list[str] = []
        try:
            cfg = yaml.safe_load(PORTFOLIO_YAML.read_text(encoding="utf-8")) or {}
            hold_syms = [str(h.get("symbol") or "") for h in (cfg.get("holdings") or [])]
            watch_syms = [str(w.get("symbol") or "") for w in (cfg.get("watchlist") or [])]
        except Exception:
            pass

        def _row(sym: str, group_label: str | None = None) -> str | None:
            rec = by_sym.get(sym)
            if not rec:
                return None
            f5 = rec.get("foreign_5d") or 0
            f20 = rec.get("foreign_20d") or 0
            t5 = rec.get("trust_5d") or 0
            d5 = rec.get("dealer_5d") or 0
            fstreak = rec.get("foreign_streak") or 0
            tstreak = rec.get("trust_streak") or 0

            bits = [f"外資5日={f5:+,}", f"外資20日={f20:+,}"]
            if abs(t5) > 0:
                bits.append(f"投信5日={t5:+,}")
            if abs(d5) > 0:
                bits.append(f"自營5日={d5:+,}")
            # Streak hints
            if abs(fstreak) >= 3:
                verb = "連買" if fstreak > 0 else "連賣"
                bits.append(f"外資{verb} {abs(fstreak)} 日")
            if abs(tstreak) >= 3:
                verb = "連買" if tstreak > 0 else "連賣"
                bits.append(f"投信{verb} {abs(tstreak)} 日")
            return f"- {sym}: {' · '.join(bits)}"

        # Priority: holdings first, then watchlist, then other (cap to keep
        ...
        if watch_syms:
            ...
            for s in watch_syms:
                ...
                if line:
                    ...
                    seen.add(s)
            out.append("")
        # Other symbols (opportunities from yesterday's analysis)
        other = [s for s in by_sym.keys() if s not in seen]
        other_lines = []
        for s in other:
            line = _row(s)
            if line:
        ...
        if other_lines:
            out.append("**其他（昨日機會清單 / 覆蓋補全）：**")
            out.extend(other_lines[:40])  # cap to 40 to keep prompt manageable
            out.append("")
...
{trimmed}
{coverage_block}{valuation_block}{chips_block}---
