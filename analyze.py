...
根據使用者組合的：(1) 三柱配置 vs 目標、(2) 單一持股集中度、(3) 現金比例、(4) 波動率 / 回撤，判斷健康度（良好 / 需調整 / 高風險）。然後點出最需要解決的 key_issue（例如「growth 柱 100%，完全沒 defense 緩衝」），再給 rebalance_advice（具體行動，例如「考慮用 10% 現金買入高息 ETF 00878 或 00919 填 defense 柱」）。
